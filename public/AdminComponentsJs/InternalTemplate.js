app.controller('InternalTemplate', function($scope, $filter, $http, $interval) {
    ...
    $scope.AssignSequence = function(item, modaltype) {
        ...
        timer = $interval(function() { console.log(iterate);
            iterate++; if (iterate >= 1) { $scope.addModalHeight(iterate + "00px"); }; if (iterate >= 4) { $interval.cancel(timer);
    ...
    $scope.ModalRequest = function() {
        var Req_Url = '/Latitude/public/admin/customer/fields/crud';
    ...
    $scope.ModalRequest2 = function() {
        var Req_Url = '/Latitude/public/admin/worker/fields/perm';
        var Pipeline_Data = {
            sequence: global_sequence,
